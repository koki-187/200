...
既知の問題と今後の対応:
- 🚧 **ファイル管理機能**: APIは実装済み、フロントエンドUIへの統合が必要
- 🚧 **メッセージ機能**: APIは実装済み、フロントエンドUIへの統合が必要  
- 🚧 **OCR機能**: APIは実装済み、案件作成/編集ページへのUI統合が必要
...
#### 🚧 バックエンド実装済み・フロントエンド統合待ち
1. **ファイル管理機能**
   - API: `/api/files/deals/:dealId` (一覧取得)
   - API: `/api/r2/upload` (アップロード)
   - フロントエンド: 案件詳細ページの「ファイル」タブにプレースホルダーのみ

2. **メッセージ機能**
   - API: `/api/messages/deals/:dealId` (一覧取得・作成)
   - フロントエンド: 案件詳細ページの「メッセージ」タブにプレースホルダーのみ

3. **OCR機能**
   - API: `/api/ocr/extract` (画像からテキスト抽出)
   - フロントエンド: 案件作成/編集ページにUI未統合

これらの機能を使用するには、フロントエンドUIの追加実装が必要です。
