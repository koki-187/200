...
<body>
  ...
  <script>
    ...
    // ========================================
    console.log('[CRITICAL DEBUG] ========== SCRIPT START v3.145.0 ==========');
    ...
    window.processMultipleOCR = async function processMultipleOCR(files) {
      ...
      try {
        ...
        if (/iPhone|iPad|iPod/.test(navigator.userAgent)) {
          ...
          alert('OCR処理エラー  ' + errorTitle + ' ' + errorMessage + '  詳細はSafari開発者ツールのコンソールを確認してください。');
        }
      }
    }
    ...
    window.autoFillFromReinfolib = async function autoFillFromReinfolib() {
      ...
      try {
        ...
        if (!properties || properties.length === 0) {
          alert(metadata.prefectureName + metadata.cityName + 'のデータが見つかりませんでした。 別の住所で試してください。');
          return;
        }
        ...
        if (filledCount > 0) {
          alert('✅ ' + filledCount + '項目を自動入力しました  入力項目: ' + filledFields.join(', ') + '  データ元: 不動産情報ライブラリ（' + metadata.year + '年第' + metadata.quarter + '四半期）  ※ハザード情報も取得しました（下部に表示）');
        } else {
        ...
        // タイムアウトエラー
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
          alert('❌ タイムアウトエラー  サーバーの応答に時間がかかりすぎています。 もう一度お試しください。');
        }
        // ネットワークエラー
        else if (error.message === 'Network Error' || !error.response) {
          alert('❌ ネットワークエラー  サーバーに接続できませんでした。 インターネット接続を確認してください。');
        }
        // 認証エラー
        else if (error.response?.status === 401) {
          alert('❌ 認証エラー  MLIT_API_KEYが設定されていないか、認証トークンが無効です。  ページを再読み込みしてログインし直してください。');
        } 
        // 400エラー（住所解析失敗）
        else if (error.response?.status === 400) {
          const message = error.response?.data?.message || '住所の解析に失敗しました';
          const details = error.response?.data?.details;
          let alertMessage = '❌ エラー  ' + message + '  正しい形式で入力してください（例: 東京都板橋区蓮根三丁目17-7）';
          if (details) {
            alertMessage += '  入力された住所: ' + details.address;
          }
          alert(alertMessage);
        } 
        // 404エラー（データなし）
        else if (error.response?.status === 404) {
          alert('❌ エラー  指定された住所のデータが見つかりませんでした。  別の住所で試してください。');
        } 
        // その他のエラー
          ...
          alert('❌ データの取得に失敗しました  エラー: ' + error.message + '  ステータス: ' + (error.response?.status || '不明') + '  詳細はコンソールを確認してください。');
        }
      } finally {
        btn.disabled = false;
        btn.innerHTML = originalHTML;
  ...
  <script src="/static/ocr-init.js?v=3.145.0"></script>
  <!-- イベント委譲パターン - インラインロジックより前に実行 -->
  <script src="/static/deals-new-events.js?v=3.145.0"></script>
