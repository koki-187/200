...
### 本番環境URL
- **Production URL (Latest v3.66.0)**: https://d2d5bf0d.real-estate-200units-v2.pages.dev 🆕 **← 最新（フロントエンドUI実装完了）**
  - ✨ **初回6情報必須マークUI**（案件作成フォームに12個の必須フィールドを明示、赤い*マーク付き）
  - ✨ **リアルタイムバリデーション**（入力中にエラーをリアルタイムフィードバック）
  - ✨ **建築基準法情報表示UI**（案件詳細ページに該当法規を自動表示、青バッジ付き）
  - ✨ **特別案件申請UI**（エージェントがクライテリア非該当案件を申請可能）
  - ✨ **特別案件承認ページ**（管理者用、承認/却下フロー完備）
  - 🎨 **ナビゲーション更新**（特別案件承認リンク追加）
